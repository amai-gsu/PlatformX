...
class ReLUConvBN(nn.Module):
    ...
    def forward(self, x):
        # print(f"Input shape to ReLUConvBN: {x.shape}")
        x = self.op(x)
        # print(f"Output shape from ReLUConvBN: {x.shape}")
        return x
    
class ReLUConvBNenergy(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, padding):
        super(ReLUConvBNenergy, self).__init__()
        self.op = nn.Sequential(
            nn.ReLU(),
...
class FactorizedReduceenergy(nn.Module):
    def __init__(self, C_in, C_out):
        super(FactorizedReduceenergy, self).__init__()
        self.relu = nn.ReLU()
        self.conv_1 = nn.Conv2d(C_in, C_out // 2, 1, stride=2, padding=0, bias=False)
        self.conv_2 = nn.Conv2d(C_in, C_out // 2, 1, stride=2, padding=0, bias=False)

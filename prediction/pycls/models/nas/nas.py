...
from pycls.models.nas.operations import FactorizedReduce, FactorizedReduceenergy
from pycls.models.nas.operations import OPS
from pycls.models.nas.operations import ReLUConvBN, ReLUConvBNenergy
...
# class Cell_energy(nn.Module):
#     """NAS cell (ported from DARTS)."""

#     def __init__(self, genotype, C_prev_prev, C_prev, C, ks, s, reduction, reduction_prev):
#         super(Cell_energy, self).__init__()
#         logger.info('{}, {}, {}'.format(C_prev_prev, C_prev, C))

#         if reduction_prev:
#             self.preprocess0 = FactorizedReduce(C_prev_prev, C)
#         else:
#             self.preprocess0 = ReLUConvBN(C_prev_prev, C, ks, s, 0)
#         self.preprocess1 = ReLUConvBN(C_prev, C, ks, s, 0)
...
class Cell_energy(nn.Module):
    """NAS cell (ported from DARTS), adjusted for dynamic configuration."""
    def __init__(self, genotype, C_prev_prev, C_prev, C, ks, s, reduction, reduction_prev, device):
        super(Cell_energy, self).__init__()
        self.device = device

        padding = (ks - 1) // 2  # Common padding formula for 'same' padding

        if reduction_prev:
            self.preprocess0 = FactorizedReduceenergy(C_prev_prev, C).to(self.device)
        else:
            self.preprocess0 = ReLUConvBNenergy(C_prev_prev, C, ks, s, padding).to(self.device)

        self.preprocess1 = ReLUConvBNenergy(C_prev, C, ks, s, padding).to(self.device)
        ...
        if reduction:
            ...
            concat = genotype['normal_concat']
        self._compile(C, op_names, indices, concat, reduction)
        self.output_channels = C * len(concat)  # 根据 concat 的长度设置输出通道数
    ...
    def _compile(self, C, op_names, indices, concat, reduction):
        ...
        for name, index in zip(op_names, indices):
            stride = 2 if reduction and index < 2 else 1
            op = OPS[name](C, stride, True).to(self.device)
            self._ops.append(op)
        self._indices = indices
    ...
    def forward(self, s0, s1, drop_path_prob):
        ...
        for i in range(self._steps):
            ...
            if self.training and drop_path_prob > 0.:
                ...
                h1 = dropout(h1, p=drop_path_prob, training=True)
                h2 = dropout(h2, p=drop_path_prob, training=True)
            states.append(h1 + h2)
